...
def test_pwa_resources_are_served_from_root_scope() -> None:
    ...
    scripts = client.get("/static/app.js")
    ...
    assert 'CACHE_NAME = "anki-shell-v18"' in service_worker.text
    assert '"/static/app.css?v=18"' in service_worker.text
    assert '"/static/app.js?v=18"' in service_worker.text
    ...
    assert 'form.matches("[data-unlike-form]")' in scripts.text
    assert "form.closest(\"article\")?.remove()" in scripts.text
